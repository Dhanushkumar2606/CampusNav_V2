...
def assistant_query(
    ...
    if intent in ("route_between", "distance", "distance_between"):
        ...
        # "How far is X?" without an explicit origin — assume the campus
        # Main Gate (a real surveyed node), and say so in the text. Never
        # fabricates the user's position.
        assumed_main_gate = origin_hit is None
        source = (origin_hit.get("slug") or origin_hit.get("id")) if origin_hit else "main_gate"
        origin_label = origin_hit["label"] if origin_hit else "Main Gate"
