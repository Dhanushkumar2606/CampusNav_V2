...
export async function searchCampus(
  ...
  return unwrap<SearchResult[]>(await fetchWithTimeout(`${API_BASE}/api/search?${params.toString()}`));
}

export async function getCampusCategories(
  campusSlug: string,
): Promise<CategoryOut[]> {
  return unwrap<CategoryOut[]>(await fetchWithTimeout(`${API_BASE}/api/campuses/${encodeURIComponent(campusSlug)}/categories`));
}

export async function getBuildingDetail(buildingId: string): Promise<BuildingDetailOut> {
  return unwrap<BuildingDetailOut>(await fetchWithTimeout(`${API_BASE}/api/buildings/${encodeURIComponent(buildingId)}`));
}

/* ----- Favorites ----- */

export async function listFavorites(token: string): Promise<FavoriteOut[]> {
  return unwrap<FavoriteOut[]>(await fetchWithTimeout(`${API_BASE}/api/favorites`, { headers: authHeaders(token) }));
}
...
export async function addFavorite(
  ...
  return unwrap<FavoriteOut>(
    await fetchWithTimeout(`${API_BASE}/api/favorites`, {
      method: "POST",
...
export async function removeFavorite(token: string, favoriteId: string): Promise<void> {
  await unwrap<void>(await fetchWithTimeout(`${API_BASE}/api/favorites/${encodeURIComponent(favoriteId)}`, {
    method: "DELETE",
    headers: authHeaders(token),
  }));
}
...
export async function getPreferences(token: string): Promise<PreferencesOut> {
  return unwrap<PreferencesOut>(await fetchWithTimeout(`${API_BASE}/api/preferences`, { headers: authHeaders(token) }));
}
...
export async function updatePreferences(
  ...
  return unwrap<PreferencesOut>(
    await fetchWithTimeout(`${API_BASE}/api/preferences`, {
      method: "PUT",
      headers: authHeaders(token),
      body: JSON.stringify(payload),
    }),
