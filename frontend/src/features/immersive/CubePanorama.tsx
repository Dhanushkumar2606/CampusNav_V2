...
export function CubePanorama({
    ...
    (async () => {
      for (const level of LEVELS_DESC) {
        ...
        try {
          ...
          return; // a deeper level failed: keep the last good quality
        }
  ...
  const onDeviceOrientation = useCallback((e: DeviceOrientationEvent) => {
    ...
    if (!vr.diagLast) vr.diagLast = 0;
    if (performance.now() - vr.diagLast > 500) {
      vr.diagLast = performance.now();
      setVrDiag(
        `ev:${(e.alpha | 0)}°/${(e.beta | 0)}° · yaw:${(vr.targetYaw * 180 / Math.PI) | 0}° pitch:${(vr.targetPitch * 180 / Math.PI) | 0}° · cal:${vr.calibrated ? "ok" : "pending"}`,
      );
    }
  }, []);
  ...
  const enterVr = useCallback(async () => {
    ...
    try {
      ...
      if (typeof DOE !== "undefined" && DOE.requestPermission) {
        console.log("[VR] requestPermission exists, requesting…");
        const res = await DOE.requestPermission();
        console.log("[VR] requestPermission result:", res);
        if (res !== "granted") {
          setVrDenied(true);
          setVrDiag(`perm:${res}`);
          return;
        ...
        console.log("[VR] no requestPermission API (desktop?)");
      }
    } catch (err) {
      console.log("[VR] requestPermission threw:", String(err));
    ...
    console.log("[VR] listeners attached, vr.active=", vr.active, "alpha/beta test→", typeof DeviceOrientationEvent);
    setVrDenied(false);
    setVrMode(true);
  ...
  return (
    ...
    >
      ...
      {/* Quality chip while sharper levels stream in. */}
      {ready && quality !== 0 ? (
        <div className="pointer-events-none absolute top-3 left-3 rounded-full bg-black/50 px-3 py-1 text-[11px] text-brand-subtle backdrop-blur">
          Improving quality…
        </div>
      ) : null}
      ...
      {/* Interaction hint. */}
      {!vrMode ? (
        <div className="pointer-events-none absolute bottom-3 left-1/2 -translate-x-1/2 rounded-full bg-black/50 px-3 py-1 text-[11px] text-brand-subtle backdrop-blur">
      ...
      {/* TEMP DEBUG: VR diagnostic readout — remove after investigation. */}
      {vrMode && vrDiag ? (
        <div className="pointer-events-none absolute top-14 right-3 z-10 max-w-[70%] rounded-lg bg-black/80 px-3 py-2 font-mono text-[10px] leading-relaxed text-lime-300 backdrop-blur">
          {vrDiag}
        </div>
      ) : null}
    </div>
